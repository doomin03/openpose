...
input_model = api.model('Input',{
    ...
    "secret_key": fields.String(required=True),
    "bucket_name": fields.String(required=True),
    "s3_key": fields.String(required=True),  # S3에서 다운로드할 객체의 키
    "local_path": fields.String(required=True),  # 로컬에 저장할 파일 경로
    "output_path": fields.String(required=True),  # 처리된 파일을 저장할 경로
})
